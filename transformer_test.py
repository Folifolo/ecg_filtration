...
def CnnTransformerModel():
    i = Input(shape=(4096, 1))

    x = Conv1D(64, 20, activation="relu", padding='same')(i)
    x = MaxPool1D(2)(x)
    ...
    x = UpSampling1D(2)(x)
    x = Conv1D(6, 20, activation='softmax', padding='same')(x)
...
if __name__ == "__main__":
    ...
    model.summary()
    plot_model(model, "pics\\transformer.png")
    X = load_split()
    train_eval(model, X, only_eval=False, save_path="transformer", size=4096, epochs=10)
